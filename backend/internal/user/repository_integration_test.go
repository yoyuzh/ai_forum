...
func TestSQLRepositoryCreateFindAndAuthenticate(t *testing.T) {
	db := newUserIntegrationDB(t)
	repo := NewSQLRepository(db)
	svc := NewService(repo)
	ctx := context.Background()

	u, err := svc.Register(ctx, RegisterInput{Username: "bob", Password: "secret123", DisplayName: "Bob"})
	if err != nil {
		t.Fatal(err)
	}
	...
	if found.Username != "bob" || found.PasswordHash == "" {
		t.Fatalf("found = %#v", found)
	}
}

func newUserIntegrationDB(t *testing.T) *sqlx.DB {
	t.Helper()
